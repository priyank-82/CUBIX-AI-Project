...
import heapq, time
from .Cubeai import Cubeai
from .Heuristic import Heuristic,myHeuristic
...
class BFS:
    ...
    def solve(self, timeout=float('inf')):
        ...
        goal_state = Cubeai(self.cube.size).__hash__()
        depth = 0
...
class A_Star:
    ...
    def solve(self, timeout=float('inf')):
        ...
        goal_state = State(Cubeai(self.cube.size), None, 0, 0, None)
        explored = set()
        fringe = [start_state]
